...
class LnClient:
    ...
    def updatechanpolicy(self, point, rate):
        return self._run("updatechanpolicy", "--base_fee_msat", "10000", "--fee_rate", "%0.8f" % rate, "--time_lock_delta", "144", "--chan_point", point)
...
for c in sorted(ln.listchannels(), key=lambda x: int(x["local_balance"]) / (int(x["local_balance"]) + int(x["remote_balance"]))):
    active, pubkey, point = c["active"], c["remote_pubkey"], c["channel_point"]
    capacity, outbound, inbound = int(c["capacity"]), int(c["local_balance"]), int(c["remote_balance"])
    ...
    table.add_row('{:,}'.format(inbound), bar, '{:,}'.format(outbound), str(fee_local), str(fee_remote), str(uptime), alias)
